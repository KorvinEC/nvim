" Vim compiler file
" Compiler:     Uv Pytest (Python testing framework)
...
if has('unix')
  execute $'CompilerSet makeprg=/usr/bin/env\ PYTHONWARNINGS=ignore\ uv\ run\ pytest\ {escape(get(b:, 'pytest_makeprg_params', get(g:, 'pytest_makeprg_params', '--tb=short --quiet')), ' \|"')}'
else
  CompilerSet makeprg=uv\ run\ pytest\ --tb=short\ --quiet
